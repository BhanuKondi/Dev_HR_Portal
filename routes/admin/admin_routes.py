...
from utils.authz import ROLE_ADMIN, ROLE_USER, get_role_id, has_manager_access, normalize_role_name, require_roles
...
def employees():
    ...
    roles = Role.query.filter(Role.name.in_(["Admin", "User", "admin", "user"])).order_by(Role.name.asc()).all()
    managers = (
        Employee.query
...
def view_employee(id):
    ...
    return jsonify({
        ...
        "role_name": normalized_role.title() if normalized_role else "",
