...
def get_consumed_leaves(emp_code, leave_type):
    ...
    total = db.session.query(func.sum(Leavee.total_days)).filter(
...
def get_available_sl(emp_code):
    return 6 - get_consumed_leaves(emp_code, "Sick Leave")
...
def submit_leave():
    ...
    if request.form.get("is_half_day") == "true":
        total_days = 0.5
    ...
    try:
        ...
        if l1_email:
            send_email(
                ...
                [l1_email],
                f"{emp.first_name} applied leave from {start} to {end}"
            )
...
def approve_leave(leave_id):
    ...
    if leave.status == "PENDING_L1":
        ...
        try:
            ...
            if l2_email:
                send_email("L2 Approval Needed", [l2_email], "Please review leave")
        except Exception as e:
            print("Email error:", e)
        ...
        try:
            ...
            if emp_email:
                send_email("Leave Approved", [emp_email], "Your leave is approved")
        except Exception as e:
            print("Email error:", e)
...
def reject_leave(leave_id):
    ...
    try:
        ...
        emp_email = get_user_email(emp_record.user_id)
        if emp_email:
            send_email("Leave Rejected", [emp_email], "Your leave was rejected")
    except Exception as e:
        print("Email error:", e)
